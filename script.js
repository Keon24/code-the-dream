   ...
   input.addEventListener('keyup', async () => {
       ...
       .then(jsonData => {
           ...
           jsonData.data.results.forEach(result => {
               ...
               div.setAttribute("onclick", `displayWords('${result.name}')`);
               div.innerHTML = `<p class="item"><b>${result.name.substr(0, input.value.length)}</b>${result.name.substr(input.value.length)}</p>`;
